...
class TemporalUnet(ModelMixin, ConfigMixin):
    ...
    def forward(self, x, cond, time):
        ...
        for resnet, resnet2, attn, upsample in self.ups:
            # print("torch.cat((x, h.pop()), dim=1)", x.shape, h[-1].shape)
            x = torch.cat((x, h.pop()), dim=1)
            # print("torch.cat((x, h.pop()), dim=1)", x.shape)
            x = resnet(x, t)
            # print("x = resnet(x, t)", x.shape)
            x = resnet2(x, t)
            # print("x = resnet2(x, t)", x.shape)
            x = attn(x)
            # print("x = attn(x)", x.shape)
            x = upsample(x)
            # TODO: hack to fix the shape
            x = x[:, :, :h[-1].shape[-1]] if h else x
            # print("x = upsample(x)", x.shape)
